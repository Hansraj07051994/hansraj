...
const Index = ({ children }) => {
  ...
  const handleDownloadResume = () => {
    ...
    const resumeUrl = "images/hansraj.pdf";
  ...
  return (
    <>
      <div className="mb-5">
        <div className="border-2 border-inherit border-solid mx-auto mt-10 rounded-xl " style={{width:"87%"}}>
          <nav className="flex flex-col md:flex-row justify-between items-center px-4 py-4 md:px-10">
            <img
              src="logo.png"
              alt="logo"
              style={{ width: "100px", height: "auto" }}
            />
            <div className={`menu-icon md:hidden ${isMenuOpen ? "hidden" : ""}`} onClick={toggleMenu}>
              <i className="ri-menu-line text-2xl"></i>
            </div>
            <ul className={`nav-links md:flex flex-col gap-4 md:flex-row md:gap-4 ${isMenuOpen ? "flex" : "hidden"}`}>
              {nav.map((data, index) => (
                <li
                  key={index}
                  className={`text-xl font-semibold text-black ${
                    activeLink === data.path ? "text-blue-500" : ""
                  }`}
                >
                  <Link to={data.path} onClick={() => handleNavLinkClick(data.path)}>
                    {data.text}
                  </Link>
                </li>
              ))}
        ...
        <div className="mx-auto w-full flex flex-row justify-between px-4 py-4 md:px-10">
          <div>
            <h1 className="text-black text-xl font-semibold">
              Copyright © 2024 Hansraj - All Rights Reserved
            </h1>
          </div>
